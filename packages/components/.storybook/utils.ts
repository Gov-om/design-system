import countries from 'flag-icons/country.json';
import breakpoints from '../src/styles/preprocessed/breakpoints.json';
import { allModes } from './modes';
import { css } from 'lit';
import { InputType } from 'storybook/internal/types';

export const countriesList = countries.filter((country) => country.name !== 'Israel').map((country) => `${country.name}`);

export const convertBreakpointsToViewports = (breakpoints: { name: string; value: number }[]) => {
  const result: any = {};

  for (const item of breakpoints) {
    const newItem = {
      name: item.name,
      styles: {
        width: item.value + 'px',
        height: '100%',
      },
      type: 'mobile',
    };

    result[item.name] = newItem;
  }

  return result;
};

export const breakpointViewports = convertBreakpointsToViewports(breakpoints);

export const applyBreakpoint = (viewport: string, returnValue: 'all' | 'globals' | 'parameters' = 'all') => {
  const globals = {
    viewport: {
      value: viewport,
    },
  };

  const parameters = {
    chromatic: {
      modes: {
        [viewport]: allModes[viewport],
      },
    },
  };

  const all = {
    globals,
    parameters,
  };

  return returnValue === 'all' ? all : all[returnValue];
};

export const storybookFloatingUIArgPlacement = {
  // We need to add the @floating-ui/dom's Placement type verbatim because cem-expanded-types does not support loading types from node_modules yet
  options: ['top', 'top-start', 'top-end', 'right', 'right-start', 'right-end', 'bottom', 'bottom-start', 'bottom-end', 'left', 'left-start', 'left-end'],
  control: { type: 'select' },
};

export const storybookArgAttachment: InputType = {
  options: ['top', 'top-start', 'top-end', 'center', 'center-start', 'center-end', 'bottom', 'bottom-start', 'bottom-end'],
  name: 'Attachment',
  description: 'Positioning of the element on screen',
  control: { type: 'inline-radio' },
  table: {
    category: 'Playground',
  },
};

export const htmlAddonTransformSource = (code: string) => {
  // Remove <storybook-comment> tags and their contents:
  return code.replace(/<storybook-comment[\s\S]*?<\/storybook-comment>/g, '');
};

export const htmlAddonTransformSourceStripAll = (code: string) => {
  // Remove <storybook-comment> tags and their contents:
  return htmlAddonTransformSource(code)
    .replace(/style="[^\\"]*"/g, '')
    .replace(/tabindex="[^\\"]*"/g, '')
    .replace(/class="sb-attachment-canvas[^\\"]*"/g, '')
    .replace(/storybookattachment="[^\\"]*"/g, '')
    .replace(/role="[^\\"]*"/g, '');
};

export const randomId = (prefix = 'storybook_') =>
  Math.random()
    .toString(36)
    .replace('0.', prefix || '');

export const storybookGUPLogo = css`'data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 97 124" fill="%2393a4dd"%3E%3Cpath fill-rule="evenodd" clip-rule="evenodd" d="M0 124H96.9991V58.9726C96.9991 44.6595 90.1315 31.0192 78.6269 22.4855L48.2851 0L18.2141 22.4946C6.80885 31.0282 0 44.6234 0 58.8642V124ZM6.00064 118.004H91.0029V58.9771C91.0029 46.5559 85.0429 34.7171 75.0599 27.3077L48.3031 7.48163L21.8082 27.2987C11.911 34.699 6.00064 46.5062 6.00064 58.8687V118.004ZM41.9052 38.1759L42.131 39.7608C45.3683 38.4468 46.6055 40.7947 47.0796 43.7612C47.3776 45.626 47.3776 47.73 47.3776 49.3284C47.3776 53.4643 42.5644 56.2998 42.5644 56.2998V59.5101H55.9564V56.2998C55.9564 56.2998 51.1432 53.4643 51.1432 49.3284C51.1432 47.73 51.1432 45.626 51.4412 43.7612C51.9153 40.7947 53.1525 38.4468 56.3898 39.7608L56.6156 38.1759C56.6156 38.1759 53.6988 34.1574 49.2604 34.1574C44.822 34.1574 41.9052 38.1759 41.9052 38.1759ZM27.154 86.3659L24.3366 89.7748C29.2175 94.0552 36.4417 97.2158 43.3996 96.5431C49.6621 95.938 54.8861 91.021 55.6898 82.8802C55.7047 82.7258 55.7208 82.5721 55.737 82.4182L55.737 82.4177C55.7946 81.8696 55.8524 81.3196 55.8524 80.731V63.7043H42.5146V85.061C35.2153 87.363 30.5665 86.7854 27.3623 86.3872L27.154 86.3614V86.3659ZM10.1908 113.746C16.5752 108.495 22.788 103.109 28.748 97.3383L28.7435 97.3338C30.0348 98.2369 31.7686 99.0586 33.7147 99.5282C28.6622 104.477 23.3118 109.191 17.8801 113.801H10.1908V113.746ZM58.8007 87.8514C58.4305 89.5852 57.6674 91.5719 56.7012 93.1161C63.4604 100.015 71.0955 107.095 78.9383 113.8H86.7585C76.8658 105.686 67.7091 97.2474 58.8007 87.8469V87.8514ZM60.0605 72.8464L60.0559 72.8521H60.0605V72.8464ZM60.0605 72.8464C67.0445 64.0829 75.5082 51.8769 82.0763 41.4942C81.1598 39.8959 80.1439 38.3607 78.988 36.9159C73.4389 45.8153 65.9708 56.9135 60.0605 64.5487V72.8464ZM38.3066 72.9062V64.6118C32.2382 56.5568 24.3502 44.9709 18.7514 35.9045C17.573 37.2816 16.5119 38.74 15.5637 40.2751C22.2417 50.9535 31.0282 63.6456 38.3066 72.9107V72.9062ZM12.4303 71.2178H29.1724V80.4016H12.4303V71.2178ZM84.6366 71.2178H67.8944V80.4016H84.6366V71.2178Z" /%3E%3C/svg%3E%0A'`;
