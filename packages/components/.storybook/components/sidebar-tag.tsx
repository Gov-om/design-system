/**
 * Original version: https://github.com/varya/storybook-8-sidebar-statuses
 ...
 * it lacks additional metadata, such as tags that signify the status of the component (e.g., WIP, BETA, DEPRECATED).
 *
 ...
 *   component: Btn,
 *   tags: ['WIP'], // Assigning status as tags here
 *   argTypes: {...},
 * };
 * ```
 *
 * These tags then serve as indicators of the component's status, providing immediate visibility into their lifecycle stage
 * right from the Storybook UI. The full list of statuses, which can be found later in the code, includes WIP, BETA, and
 * DEPRECATED.
 *
...
const getTags = (data: DataMap, itemId: string): string[] => {
  ...
  if (filteredTags.length === 0) {
    ...
    const entry = Object.entries(data).find(([key, value]) => key.startsWith(`${itemId}--`) && filterStatusTags(value.tags).length > 0);
    ...
    {(stories) => {
      ...
      return tags.map((tag: string) => {
        ...
        return (
          <span
            key={tag}
            style={{ ...style, marginLeft: 'auto', marginRight: '15px', borderRadius: '3px', padding: '0 4px', display: 'inline-block', fontSize: '11px' }}>
            {title}
          </span>
        );
      });
